...
void msm_isp_start_error_recovery(struct vfe_device *vfe_dev)
{
	...
	atomic_cmpxchg(&vfe_dev->error_info.overflow_state,
		NO_OVERFLOW, OVERFLOW_DETECTED);
	vfe_dev->at_halt_irq0_mask = vfe_dev->irq0_mask;
	vfe_dev->at_halt_irq1_mask = vfe_dev->irq1_mask;
	...
	if (vfe_dev->is_split) {
		...
		other_vfe_dev = vfe_dev->common_data->
			dual_vfe_res->vfe_dev[other_vfe_id];
		other_vfe_dev->at_halt_irq0_mask = other_vfe_dev->irq0_mask;
		other_vfe_dev->at_halt_irq1_mask = other_vfe_dev->irq1_mask;
